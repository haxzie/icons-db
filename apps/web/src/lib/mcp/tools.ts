...
export function registerTools(server: McpServer, origin: string) {
  server.registerTool(
    "search_icons",
    {
      ...
      description:
        ...
        "Searches 200k icons across 81 sets with hybrid keyword + semantic ranking. " +
  ...
  server.registerTool(
    "list_icon_sets",
    {
      ...
      description: "All 81 icon/logo/emoji sets with counts, license, attribution flag and npm packages. Filter by kind, license or package.",
      inputSchema: z.object({
        kind: KIND.optional(),
