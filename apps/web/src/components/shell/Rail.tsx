  ...
  return (
    <nav className="sticky top-0 z-40 flex h-screen w-[72px] shrink-0 flex-col items-center border-r bg-bg py-3 max-md:hidden">
          ...
          <Link key={it.href} href={it.href} className="group flex w-full flex-col items-center gap-1 py-2 text-[11px] font-medium text-fg-muted">
            <span
              className={`grid h-8 w-14 place-items-center rounded-full transition ${
                active ? "bg-accent-soft text-accent dark:text-[#d2e3fc]" : "group-hover:bg-bg-muted"
              }`}
            >
              {it.icon}
            </span>
      ...
      <div className="mt-auto flex flex-col items-center gap-2">
        <a href="https://github.com/haxzie/icons-db" target="_blank" rel="noreferrer" aria-label="GitHub" className="grid size-9 place-items-center rounded-full text-fg-muted hover:bg-bg-muted">
